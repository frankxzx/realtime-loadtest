...
用法:
  ...
  # 1006 随机断连排查：100 路整通长电话(不重连)，每轮后静默 45s 模拟坐席讲话，
  # 跑 30 分钟攒连接·小时数；断连自动记录 close code/阶段/存活时长+同时刻成簇检测
  python3 realtime_loadtest.py --mode chat --concurrency 100 --session-loop \\
      --turn-gap 45 --duration 1800 --html

  # 本地复现 1006(带病客户端 A/B 对照，配 mock_gateway.py，详见 README)：
  #   终端1: python3 mock_gateway.py --port 9800
  ...
  # B 带病实验组(bytes+=累积 + 同步转码，预期 lag 飙升 + 1006 成簇)：
  python3 realtime_loadtest.py --mode chat --concurrency 8 --session-loop \\
...
chat 模式(保险坐席对话)关键参数:
  ...
  --session-loop      同一条 WS 循环话本到 duration 不重连(整通长电话)，钓随机断连
  --turn-gap N        每轮回复后静默 N 秒(坐席讲话中，WS 空闲只剩 keepalive)
  --ping-interval/--ping-timeout  WS keepalive 参数(默认 20/20s)，排查断连用
  不加 --sync-fire    持续模式：每 worker 循环整段话本(每轮带全量历史上下文)到 duration

1006 断连取证: 中途断连按 close code 归类(没收到 close frame=1006)，记录断连时
所处阶段(send_turn/awaiting_response/idle_gap)、连接存活时长、最近 keepalive RTT；
自动检测「多路同时刻断连成簇」(成簇=服务端/网关侧事件，分散=网络/keepalive/单连接)；
monitor 带事件循环滞后探针，排除压测机过载自伤。断连率按连接·小时归一化(summary
的 abnormal_per_conn_hour)，可直接和生产断连率对比。
...
class GlobalStats:
    ...
    # WS 中途断连取证（1006 排查）：code/阶段/连接存活时长/最近 keepalive RTT
    ws_abnormal_closes:     int  = 0
    abnormal_close_details: list = field(default_factory=list)
    # 连接生命周期：断连率要按「每连接·小时」归一化才能和生产对比
    conn_opened:        int   = 0
    conn_seconds_total: float = 0.0
    # 压测机自伤探针：事件循环最大滞后（lag 大时 pong 回不及时，1006 可能是自己造成的）
    ...
    async def record_abnormal_close(self, code: int, reason: str, phase: str,
                                    conn_age: float, last_ping_ms: float | None = None):
        """WS 中途断连（建连成功之后被掐）。code=1006 表示没收到 close frame
        （TCP 被直接掐断——网关/服务端/网络/客户端 pong 超时都可能）；其余 code
        是对端体面关闭时带的码。取证三件套：断连时所处阶段、连接存活时长、
        最近一次 keepalive RTT。"""
        async with self.lock:
            elapsed = time.monotonic() - self.start_time
            b, cc, s, w = _ctx()
            self.connection_errors += 1
            self.ws_abnormal_closes += 1
            self.abnormal_close_details.append({
                "elapsed":      round(elapsed, 1),
                "batch":        b,
                "batch_cc":     cc,
                "seq":          s,
                ...
                "code":         code,
                "phase":        phase,
                "conn_age_s":   round(conn_age, 1),
                "last_ping_ms": round(last_ping_ms, 1) if last_ping_ms else None,
                "reason":       reason[:200],
            })
            self.errors.append(f"[B{b}#{s} WS_CLOSE {code}/{phase} "
                               f"存活{conn_age:.0f}s] {reason[:80]}")
            self._mark_first_anomaly_unlocked(f"ws_close_{code}",
                                              f"阶段={phase} 存活={conn_age:.1f}s {reason}",
                                              elapsed)

    def record_conn_lifetime(self, age: float) -> None:
        # asyncio 单线程、无 await，不需要锁（同 next_seq）
        self.conn_opened += 1
        self.conn_seconds_total += age

    def _death_clusters(self, window: float = 2.0) -> list[dict]:
        """跨连接的「同时刻断连」聚类：window 秒内 ≥2 路一起死。
        随机分散 = 网络/keepalive/单连接问题；成簇 = 服务端/网关侧事件
        （节点回收、扩缩容、负载丢弃）——这是归因 1006 最有力的区分证据。"""
        times = sorted(d["elapsed"] for d in self.abnormal_close_details)
        clusters, i = [], 0
        while i < len(times):
            j = i
            while j + 1 < len(times) and times[j + 1] - times[i] <= window:
                j += 1
            if j > i:
                clusters.append({"elapsed": times[i], "count": j - i + 1})
            i = j + 1
        return clusters
    ...
    def snapshot(self, elapsed: float, loop_lag_ms: float = 0.0):
        ...
        if tpm > self.peak_tpm:
            self.peak_tpm = tpm
        if loop_lag_ms > self.loop_lag_max_ms:
            self.loop_lag_max_ms = loop_lag_ms
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "ws_abnormal_closes": self.ws_abnormal_closes,
            "abnormal_close_by_code": {
                str(c): sum(1 for d in self.abnormal_close_details if d["code"] == c)
                for c in sorted({d["code"] for d in self.abnormal_close_details})
            },
            "death_clusters":     self._death_clusters(),
            "conn_count":         self.conn_opened,
            "conn_minutes":       round(self.conn_seconds_total / 60, 1),
            "abnormal_per_conn_hour": (
                round(self.ws_abnormal_closes / (self.conn_seconds_total / 3600), 2)
                if self.conn_seconds_total > 0 else 0),
...
def _ssl_arg():
    # wss 用关校验的 _SSL_CTX；本地复现连 mock（http→ws://）时必须传 None，
    # websockets 对 ws:// + ssl 参数直接报错
    return _SSL_CTX if ENDPOINT.startswith("https://") else None
...
async def run_text_session(
    ...
    try:
        async with websockets.connect(
            ...
            open_timeout=10, close_timeout=5, ssl=_ssl_arg(),
        ) as ws:
...
# ─── 带病客户端模拟（--sim-pcm-accumulate，本地复现 1006 用）────────────────────
class SimPcmClient:
    """复刻生产客户端的病灶写法，用来证明 1006 是客户端事件循环被强占导致的：

    1. 录音累积：bytes 属性 `buf += chunk`——bytes 不可变，每次 += 整段复制，
       O(n²)。24kHz/16bit 真实速率 2.88MB/min，第 7 分钟单通话就要强占事件循环
       ~10s/分钟（实测），多通并发再乘 N。
    2. 同步转码：每轮说完后模拟 ffmpeg/pydub 同步跑在事件循环上，阻塞时长 =
       缓冲大小 / encode_mbps——一次整段硬阻塞数秒，同进程所有连接一起 pong
       不出去，被网关按 keepalive 超时 RST 掐掉 → 全进程 1006 成簇。

    实验设计：同参数跑一遍不带 --sim-pcm-accumulate（干净对照组，预期 0 断连），
    再跑一遍带（实验组，预期 loop lag 飙升 + 1006 成簇）。
    """
    CHUNK_HZ = 25   # 40ms 一个 chunk，和 Realtime API 音频帧节奏一致

    def __init__(self, mb_per_min: float, encode_mbps: float):
        self.buf = b""
        self.chunk = b"\x00" * max(1, int(mb_per_min * 1e6 / 60 / self.CHUNK_HZ))
        self.encode_mbps = encode_mbps

    async def recorder(self):
        """后台“录音”任务：病灶本体。跑在事件循环上，谁都躲不开它的复制停顿。

        按墙钟时间补账：真实世界里音频帧不因事件循环卡顿而停止到达，
        卡住期间积压的帧会在解卡后被突发处理（每帧照样 +=）——这正是
        生产里“越卡越追账、越追越卡”的正反馈。"""
        step = 1 / self.CHUNK_HZ
        next_t = time.monotonic()
        while True:
            await asyncio.sleep(step)
            now = time.monotonic()
            while next_t <= now:
                self.buf += self.chunk      # O(n²)：每次 += 全量复制
                next_t += step

    def encode_sync(self) -> float:
        """模拟“说完一轮把缓冲同步转 MP3”：按 encode_mbps 忙等，占死事件循环。"""
        if self.encode_mbps <= 0 or not self.buf:
            return 0.0
        target = len(self.buf) / (self.encode_mbps * 1e6)
        t0 = time.perf_counter()
        while time.perf_counter() - t0 < target:
            zlib.crc32(self.chunk)          # 真实烧 CPU，不是 sleep
        return target
...
async def run_chat_session(
    ...
    turn_gap: float = 0.0, session_loop: bool = False,
    ping_interval: float = 20.0, ping_timeout: float = 20.0,
    sim_pcm: bool = False, sim_rate_mb_min: float = 2.88, sim_encode_mbps: float = 5.0,
) -> None:
    """
    ...
    - session_loop（--session-loop）：同一条 WS 循环话本直到 stop_event 才收工，
      不主动断开——配 turn_gap 模拟整通十几分钟的长电话，钓随机断连（1006）
    - turn_gap（--turn-gap）：每轮回复后静默 N 秒（坐席讲话中，WS 完全空闲）
    - 中途断连按 close code 归类（没收到 close frame 即 1006），记录取证三件套：
      断连时所处阶段(phase) / 连接存活时长 / 最近一次 keepalive RTT
    """
    wid = f"W{worker_id:02d}"
    scenario = CHAT_SCENARIOS[worker_id % len(CHAT_SCENARIOS)]
    reported = False
    phase = "handshake"                  # 断连取证：死的时候在干嘛
    t_conn = 0.0
    last_ping_ms: float | None = None    # ws.latency：keepalive ping 的 RTT
    sim = SimPcmClient(sim_rate_mb_min, sim_encode_mbps) if sim_pcm else None
    rec_task: asyncio.Task | None = None
    try:
        async with websockets.connect(
            build_ws_url(deployment),
            additional_headers={"api-key": API_KEY},
            ...
            if sim is not None:
                rec_task = asyncio.create_task(sim.recorder())   # 带病“录音”开跑
            phase = "session_setup"
            await _wait_event(ws, wid, "session.created", timeout=10)
            await _ws_send(ws, wid, {
                "type": "session.update",
            ...
            while not done:
                for turn in scenario["live_turns"]:
                    n_turn += 1
                    _CTX_SEQ.set(stats.next_seq())
                    # 静默期被掐(RST)时异常要等下一次 IO 才抛——连接已死就保持
                    # idle_gap 归因，别把死在静默期的算到 send_turn 头上
                    if getattr(ws, "close_code", None) is None:
                        phase = "send_turn"      # 「说完刚发送」窗口
                    await _ws_send(ws, wid, {
                        "type": "conversation.item.create",
                        "item": {"type": "message", "role": "user",
                    ...
                    LOG.trigger(wid, "response.create", f"轮{n_turn}")
                    phase = "awaiting_response"  # 「正在生成」窗口
                    await _ws_send(ws, wid, {"type": "response.create"})
                    input_tok, output_tok = await _wait_response_done(
                        ws, wid, timeout=timeout, stats=stats)
                    latency = time.monotonic() - t_turn
                    lat_s = getattr(ws, "latency", 0.0)
                    if lat_s:
                        last_ping_ms = lat_s * 1000
                    LOG.success(wid, "response.done",
                                f"轮{n_turn} in={input_tok} out={output_tok} lat={latency:.2f}s")
                    await stats.record_success(input_tok, output_tok, latency)
                    if sim is not None:
                        # 「说完一轮」同步转码：整段硬阻塞，同进程全部连接一起遭殃
                        blocked = sim.encode_sync()
                        if blocked >= 0.5:
                            LOG.warn(wid, "sim-encode",
                                     f"同步转码阻塞 {blocked:.1f}s "
                                     f"buf={len(sim.buf) / 1e6:.0f}MB")
                    if sync is not None:
                        done = True   # 齐射模式只打同步的这一轮
                        break
                    if stop_event is not None and stop_event.is_set():
                        done = True
                        break
                    if turn_gap > 0:
                        phase = "idle_gap"       # 坐席讲话中：WS 无任何上行
                        await asyncio.sleep(turn_gap)
                if not session_loop:
                    done = True

    except ConnectionClosed as e:
        # 建连成功之后被掐：rcvd 为空 = 没收到 close frame = 1006 异常断连
        conn_age = time.monotonic() - t_conn if t_conn else 0.0
        code = e.rcvd.code if e.rcvd else 1006
        reason = (e.rcvd.reason if e.rcvd else "") or str(e)
        ping_info = f" ping={last_ping_ms:.0f}ms" if last_ping_ms else ""
        LOG.error(wid, f"WS断连 code={code}",
                  f"阶段={phase} 存活={conn_age:.1f}s{ping_info} {reason[:80]}", e)
        await stats.record_abnormal_close(code, reason, phase, conn_age, last_ping_ms)
    except RateLimitError as e:
        LOG.rate_limit(wid, f"[{e.code}] {e}")
        await stats.record_rate_limit(str(e), e.code, e.retry_after, source="session")
    except ResponseFailed as e:
        ...
        if rec_task is not None:
            rec_task.cancel()
        if t_conn:
            stats.record_conn_lifetime(time.monotonic() - t_conn)
        if sync is not None and not reported:
...
async def worker_loop(
    ...
    sync: SyncFire | None = None, chat_opts: dict | None = None,
) -> None:
...
async def monitor_loop(
    ...
    while not stop_event.is_set():
        ...
        # 事件循环滞后探针：定时器实际唤醒比预期晚多少。lag 大说明压测机自身
        # 处理不过来（pong 也会回不及时），此时看到的 1006 可能是自伤，别甩锅服务端
        lag_ms = max(0.0, (time.monotonic() - t_sleep - interval) * 1000)
        elapsed = time.monotonic() - t0
        stats.snapshot(elapsed, lag_ms)
        if lag_ms > 200:
            print(f"  {_C['yellow']}⚠ 压测机事件循环滞后 {lag_ms:.0f}ms，"
                  f"本机可能过载，断连归因需谨慎{_C['reset']}")
        s    = stats.summary()
...
async def run_load_test(
    ...
    if mode == "chat":
        co = chat_opts or {}
        extra = "  [sync-fire: 全员注好历史后同一时刻齐发]" if sync_fire else ""
        if co.get("session_loop"):
            extra += "  [session-loop: 整通长电话不重连]"
        if co.get("turn_gap"):
            extra += f"  [turn-gap: 每轮后静默 {co['turn_gap']}s]"
        if co.get("sim_pcm"):
            extra += (f"  [带病客户端: +=累积 {co.get('sim_rate_mb_min', 2.88)}MB/min"
                      f" + 同步转码 {co.get('sim_encode_mbps', 5.0)}MB/s]")
        print(f"  场景: 保险坐席×AI客户，mock 历史+坐席一轮≈1分钟话术，"
              f"{len(CHAT_SCENARIOS)} 个话本轮换{extra}")
    if mode == "transcribe":
        if burst > 0:
    ...
    workers = [
        asyncio.create_task(
            worker_loop(stats, mode, deployment, stop_event, i, request_interval,
                        ...
                        pipeline, _share(i), sync, chat_opts)
        )
...
async def run_ramp_test(
    ...
    transcribe_model: str = "", language: str = "",
    reuse_conn: bool = False, connect_stagger: float = 0.0,
    pipeline: int = 1, chat_opts: dict | None = None,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--session-loop", action="store_true",
                   help="仅 chat 模式：同一条 WS 循环话本直到 --duration，不主动断开，"
                        "模拟整通十几分钟的长电话。配 --turn-gap 用来钓随机断连(1006)，"
                        "断连会记录 close code/所处阶段/连接存活时长，并检测多路同时刻"
                        "断连聚类(成簇=服务端/网关侧事件，分散=网络/keepalive)")
    p.add_argument("--turn-gap", type=float, default=0.0,
                   help="仅 chat 模式：每轮回复后的静默秒数(模拟坐席讲话，期间 WS "
                        "完全空闲、只剩 keepalive ping)。默认 0=背靠背。真实通话形态"
                        "建议 30~60")
    p.add_argument("--ping-interval", type=float, default=20.0,
                   help="chat 模式 WS keepalive ping 间隔秒(默认 20，websockets 库默认值)，"
                        "0=禁用客户端 ping(测服务端/网关会不会因无流量掐线)")
    p.add_argument("--ping-timeout", type=float, default=20.0,
                   help="chat 模式等 pong 超时秒(默认 20)，超时客户端主动断开。"
                        "排查 keepalive 类断连时配合 --ping-interval 调")
    p.add_argument("--sim-pcm-accumulate", action="store_true",
                   help="仅 chat 模式：带病客户端模拟——每连接后台任务做 bytes+=chunk "
                        "录音累积(O(n²))，每轮说完后同步'转码'硬阻塞事件循环。"
                        "复现「客户端事件循环被强占→pong 不出去→被网关掐→1006」。"
                        "同参数先跑一遍不带此开关做干净对照组")
    p.add_argument("--sim-rate-mb-min", type=float, default=2.88,
                   help="模拟录音累积速率 MB/分钟(默认 2.88=24kHz/16bit 真实速率；"
                        "本地快速复现建议 40~60，等效于把通话时间轴压缩)")
    p.add_argument("--sim-encode-mbps", type=float, default=5.0,
                   help="模拟同步转码速度 MB/s(默认 5，约为 ffmpeg 单线程 MP3 编码量级)，"
                        "阻塞时长=当前缓冲/此速度；0=只累积不转码")
...
def main() -> None:
    ...
    if args.sync_fire and args.ramp:
        ...
        sys.exit(1)
    if (args.session_loop or args.turn_gap > 0) and args.mode != "chat":
        print("错误: --session-loop / --turn-gap 仅支持 chat 模式")
        sys.exit(1)
    if args.sim_pcm_accumulate and args.mode != "chat":
        print("错误: --sim-pcm-accumulate 仅支持 chat 模式")
        sys.exit(1)
    if args.sync_fire and args.session_loop:
        print("错误: --sync-fire(齐射一轮即止) 与 --session-loop(长电话) 不能同时使用")
        sys.exit(1)
    ...
    chat_opts = {
        ...
        "session_loop":    args.session_loop,
        "ping_interval":   args.ping_interval,
        "ping_timeout":    args.ping_timeout,
        "sim_pcm":         args.sim_pcm_accumulate,
        "sim_rate_mb_min": args.sim_rate_mb_min,
        "sim_encode_mbps": args.sim_encode_mbps,
    } if args.mode == "chat" else None
    ...
    try:
        if args.ramp:
            ...
            if stats.errors:
                ...
                for e in stats.errors[-10:]:
                    print(f"  {e}")
            if stats.abnormal_close_details:
                print(f"\n{_C['red']}{_C['bold']}WS 中途断连取证 "
                      f"({stats.ws_abnormal_closes} 次):{_C['reset']}")
                for d in stats.abnormal_close_details[-15:]:
                    ping = f" ping={d['last_ping_ms']}ms" if d["last_ping_ms"] else ""
                    print(f"  +{d['elapsed']:>7.1f}s {d['worker']} code={d['code']} "
                          f"阶段={d['phase']:<17} 存活={d['conn_age_s']}s{ping} "
                          f"{d['reason'][:60]}")
                clusters = stats._death_clusters()
                if clusters:
                    print(f"  {_C['yellow']}⚑ 同时刻断连成簇(2s窗口): "
                          + "  ".join(f"+{c['elapsed']}s×{c['count']}路" for c in clusters)
                          + f" —— 指向服务端/网关侧事件{_C['reset']}")
                else:
                    print(f"  {_C['dim']}断连时间随机分散，无同时刻成簇 —— "
                          f"更像单连接层面问题(网络/keepalive){_C['reset']}")
                if stats.loop_lag_max_ms > 200:
                    print(f"  {_C['yellow']}⚠ 事件循环最大滞后 {stats.loop_lag_max_ms:.0f}ms，"
                          f"压测机可能过载，部分断连或为自伤{_C['reset']}")
        ...
        if args.html and stats:
            ...
            meta = {
                ...
                "ping_interval":   args.ping_interval,
                "ping_timeout":    args.ping_timeout,
                "abnormal_close_details": stats.abnormal_close_details,
                "region":          args.region,
                "mode":            args.mode,
